...
export default function MockExamsComparisonPage() {
  const courses = courseDetails.filter(c => c.category === "Mock Exams")

  return (
    <div className="pt-28 pb-20 px-5 max-w-7xl mx-auto">
      <Link href="/courses/mock-exams" className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline mb-8">
        <ArrowLeft className="h-4 w-4" /> Back to Mock Exams
      </Link>
      <div className="text-center mb-12">
        <Badge variant="default" className="mb-4">COMPARE</Badge>
        <h1 className="text-3xl md:text-4xl font-bold text-on-surface mb-4">Compare Mock Exams</h1>
        <p className="text-on-surface-variant max-w-2xl mx-auto">Review all mock exam options side-by-side to find the right fit for your preparation.</p>
      </div>
