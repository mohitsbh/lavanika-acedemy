...
export default function OthersComparisonPage() {
  const courses = courseDetails.filter(c => c.category === "Others")

  return (
    <div className="pt-28 pb-20 px-5 max-w-7xl mx-auto">
      <Link href="/courses/others" className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline mb-8">
        <ArrowLeft className="h-4 w-4" /> Back to Other Services
      </Link>
      <div className="text-center mb-12">
        <Badge variant="default" className="mb-4">COMPARE</Badge>
        <h1 className="text-3xl md:text-4xl font-bold text-on-surface mb-4">Compare Other Services</h1>
        <p className="text-on-surface-variant max-w-2xl mx-auto">Review all additional services side-by-side to find the right option for your needs.</p>
      </div>
