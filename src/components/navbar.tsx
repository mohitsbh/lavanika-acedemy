    ...
    <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-2xl border-b border-gray-200/60 shadow-sm">
      <div className="max-w-7xl mx-auto flex justify-between items-center h-20 px-5">
        ...
        <nav className="flex items-center">
          ...
          <button
            className="lg:hidden relative z-50 ml-2 flex items-center justify-center w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"
      ...
      {open && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white/95 backdrop-blur-2xl rounded-t-3xl border-t border-gray-200/60 shadow-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="px-5 pb-8 pt-2 flex flex-col gap-0.5">
                ...
                link.dropdown ? (
                  <div key={link.label}>
                    <Link
                      ...
                      className={cn(
                        "flex items-center justify-between py-3 px-4 text-base font-medium rounded-2xl transition-colors",
                        pathname.startsWith("/courses")
                          ? "text-primary bg-primary/10 font-semibold"
                          : "text-gray-700"
                      )}
                    ...
                    <div className="ml-4 pl-4 border-l border-gray-200/60 mb-1">
                      {link.dropdown.map((sub) => (
                        <Link
                          key={sub.href}
                          href={sub.href}
                          onClick={() => setOpen(false)}
                          className={cn(
                            "block py-2.5 px-4 text-sm rounded-xl transition-colors",
                            pathname === sub.href
                  ...
                  <Link
                    ...
                    onClick={() => setOpen(false)}
                    className={cn(
                      "py-3 px-4 text-base font-medium rounded-2xl transition-colors",
                      pathname === link.href
                        ? "text-primary bg-primary/10 font-semibold"
                        : "text-gray-700"
                    )}
              ...
              <div className="mt-4 pt-4 border-t border-gray-200/60 flex flex-col gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 py-3.5 px-4 text-base font-medium rounded-2xl bg-gray-50/50 border border-gray-200/60 text-gray-600 hover:text-primary transition-colors"
                >
                ...
                <Link
                  href="/contact"
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-center py-3.5 px-4 text-base font-semibold rounded-2xl text-white bg-primary hover:bg-primary-container transition-colors"
                >
                  Contact Us
                </Link>
              </div>
